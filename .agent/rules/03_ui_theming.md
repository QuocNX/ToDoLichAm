...
## Theming
* **Centralized Theme:** Define a centralized `ThemeData` object to ensure application-wide consistency.
* **Light/Dark Themes:** Support both themes with `ThemeMode.light`, `ThemeMode.dark`, `ThemeMode.system`.
* **Color Scheme:** Use `ColorScheme.fromSeed` to generate harmonious palettes from a single seed color.
* **Component Themes:** Customize with `appBarTheme`, `elevatedButtonTheme`, etc.
* **Custom Fonts:** Use `google_fonts` package with `TextTheme` for consistent typography.
...
## Assets and Images
* **Asset Declaration:** Declare paths in `pubspec.yaml` under `flutter: assets:`
* **Local Images:** `Image.asset('assets/images/placeholder.png')`
* **Network Images:** Use `Image.network` with `loadingBuilder` and `errorBuilder`
* **Cached Images:** Use `cached_network_image` package for caching
* **Custom Icons:** Use `ImageIcon` for custom icons from `ImageProvider`
